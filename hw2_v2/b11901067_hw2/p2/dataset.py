...
def get_dataloader(
    ...
    if split == 'train':
        transform = transforms.Compose([
            ...
            transforms.RandomRotation(degrees=10),  # random rotate 10 degrees
            transforms.RandomResizedCrop(size=32, scale=(0.9, 1.0)),  # random crop
...
class CIFAR10Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 2. 如果有 transform，則應用變換
        if self.transform:
            image = self.transform(image)

        # 3. 如果是訓練/驗證集，則返回 (image, label)
        if self.split != 'test':
            label = torch.tensor(self.labels[index], dtype=torch.long)  # 轉為 long tensor
